...
# 	EX) 학생 정보가 두 개 이상의 테이블에 저장 | 동일한 학셍이 여러 행에 중복되는 경우 

# cf) 이상 현상 
# : 비정규화된 데이터베이스에서 발생할 수 있는 데이터의 무결성 문제 
# > 삽입이상, 갱신이상, 삭제이상 

# 1) 삽입이상: 새로운 데이터 삽입 시, 비효율적이거나 불가능한 상황 
# 2) 갱신이상: 데이터의 일부를 수정할 때, 중복된 모든 데이터를 수정해야 하는 상황 
# 3) 삭제이상: 데이터 삭제 시, 그와 관련된 필요한 데이터까지 삭제되는 상황 

# 2. 정규화 종류
# 정규형: 테이블이 정규화된 정도, normal form(nf)

# : 제1정규형(1nf), 제2정규형(2nf), 제3정규형(3nf), BCNF(Boyce-Codd 정규형) 등 
...
create table `student_course`(
    ...
    # 복합 키: 두개 이상의 컬럼을 PK키로 지정하는 제약조건 
    # >> 지정된 여러 개의 컬럼이 결합되어 유일한 식별자로 존재 
    # >> 해당 각각의 컬럼이 컬럼 내에서는 중복 가능하지만 툴을 합친 값은 테이블 내에서 고유 
    #	: student_id와 course_id가 함께 기본키 역할을 함.
...
values 
    ...
    (2, 101),
    (3, 101),
    (3, 103); -- 복합키 설정이기 때문에 두개의 컬럼을 합친 내용이 다른 레코드와 중복 X
