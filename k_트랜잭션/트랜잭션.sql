...
# : 트랜잭션 연산은 모두 성공적으로 완료하거나 모두 취소되어야 함
...
# 3) 커밋(Commit) - 모든 작업이 성공적으로 완료되면, DB에 변경 사항을 확정하는 커밋 명령을 실행 
# 4) 롤백(Rollback) - 작업 중 오류 발생 | 작업을 취소하고 싶을 때 롤백 명령을 통해 DB를 트랙잭션 시작 전 상태로 되돌림 

# 트랜잭션의 예시 
# start transaction
	-- 해당 블럭 내의 명령어들은 '마치 하나의 명령어처럼 실행'
...
insert into `member` (member_id, member_name, member_age) values (1, '홍길동', 30);

insert into `buy` values (1, 1, '노트북', 200);
...
-- settings >> SQL Editor >> safe update 끄기 

## 홍길동 >> 홍길서 1000원을 이체하는 트랜잭션 
-- 자동 커밋을 헤제하여 수동 트랜잭션을 관리 
set autocommit = 0; -- 해제 
