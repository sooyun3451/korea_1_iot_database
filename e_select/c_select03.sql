...
/*
    ...
    group by: 지정된 열 데이터들 중 같은 데이터끼리 묶어서 추출
    having: 구룹화된 데이터에 조건을 추가 
*/
...
# >> 생략할 경우 기본값 0
select * from `members`
limit 5 offset 3; -- 첫번째 행의 경우 offset 0번 
...
-- 지역별 평균 포인트 계산 
select area_code, avg(points) as member_point from `members`
group by area_code;


# 5. having 
# : group by와 함꼐 사용, 그룹화 된! 결과에 대한 조건을 지정 
# >> where과 유사하지만 그룹화 된! 데이터에 대한 조건에 사용 
select grade, count(*) as member_count from `members`
group by grade having count(*) >= 2;

-- 지역별 평균 포인트 구하기 
select area_code, avg(points) as avg_points from `members`
group by area_code
